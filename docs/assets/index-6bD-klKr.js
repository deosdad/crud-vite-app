...
\r
    ...
    `;const a=document.createElement("tbody");return e.append(t,a),e},q=e=>{const t=e.target.closest(".select-user");if(!t)return;const a=t.getAttribute("data-id");w(a)},B=async e=>{const t=e.target.closest(".delete-user");if(!t)return;const a=t.getAttribute("data-id");try{await D(a),await l.reloadPage(),document.querySelector("#current-page").innerText=l.getCurrentPage(),h()}catch{alert("No se puede eliminar")}},h=e=>{const t=l.getUsers();u||(u=H(),e.append(u)),u.addEventListener("click",q),u.addEventListener("click",B);let a="";t.forEach(r=>{a+=`
            <tr>
                <td>${r.id}</td>
                <td>${r.balance}</td>
                <td>${r.firstName}</td>
